...
export default function RoomsPage() {
  ...
  const executeJoin = async (roomId: string, password: string = '') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.message || '비밀번호가 틀렸거나 입장에 실패했습니다.');
      }
    } catch (error) {
      alert('서버 오류가 발생했습니다.');
    }
  ...
  const handleCreateRoom = async (e: React.FormEvent) => {
    ...
    if (newRoom.isPrivate && newRoom.password.length < 4) return alert("비밀번호를 4자리 이상 입력해주세요.");
      ...
      <main className="mx-auto max-w-6xl px-6 mt-12 space-y-10">
        ...
        {isLoading ? (
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rooms.length === 0 ? (
              <div className="col-span-full text-center py-12 text-gray-500 font-bold">아직 개설된 모임방이 없습니다.</div>
            ) : (
              rooms.map((room) => {
                  ...
                  <div key={room.id} className="bg-white p-7 rounded-[2rem] border border-gray-100 hover:shadow-xl transition-all flex flex-col h-full relative group">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex space-x-1">
                        ...
                        {/* ⭐️ 비밀방 아이콘 추가 */}
                        {room.isPrivate && (
                          <span className="px-2 py-1 rounded bg-red-50 text-red-500"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" /></svg></span>
                        )}
                      </div>
                    ...
                    <button 
                      ...
                      className={`w-full py-3.5 rounded-xl text-sm font-black tracking-widest transition-all ${
                        room.isJoined ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : isFull ? 'bg-red-50 text-red-300' : 'bg-black text-white hover:bg-gray-800'
                      }`}
                    >
                      {room.isJoined ? "참여 완료" : isFull ? "정원 초과" : room.isPrivate ? "비밀방 참여" : "바로 참여하기"}
                    </button>
                  </div>
                );
              })
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-white rounded-[2rem] p-8 shadow-2xl">
            ...
            <div className="flex space-x-3">
              <button onClick={() => { setIsPasswordModalOpen(false); setInputPassword(''); }} className="flex-1 py-4 text-sm font-bold text-gray-400">취소</button>
              <button onClick={() => executeJoin(targetRoomId, inputPassword)} className="flex-1 py-4 bg-black text-white rounded-xl text-sm font-black shadow-lg">확인</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-8 py-6 border-b border-gray-100">
              ...
              <button onClick={() => setIsCreateModalOpen(false)} className="text-gray-400 hover:text-gray-900 transition"><svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <form onSubmit={handleCreateRoom} className="p-8 space-y-5 overflow-y-auto no-scrollbar">
              <div>
                ...
                <input type="text" placeholder="예: 주말 아침 독서 클럽" value={newRoom.title} onChange={(e) => setNewRoom({...newRoom, title: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-black font-bold" />
              </div>
              <div className="space-y-4">
                <div className={`flex items-center space-x-3 p-4 rounded-2xl border transition cursor-pointer ${newRoom.isPrivate ? 'bg-black text-white border-black' : 'bg-gray-50 border-gray-100 text-gray-900'}`} onClick={() => setNewRoom({...newRoom, isPrivate: !newRoom.isPrivate})}>
                  ...
                  <div className="flex flex-col"><span className="text-sm font-black">비밀방으로 만들기</span><span className={`text-[10px] font-bold ${newRoom.isPrivate ? 'text-gray-300' : 'text-gray-400'}`}>체크하면 비밀번호가 있어야 입장 가능합니다.</span></div>
                </div>
                {newRoom.isPrivate && (
                  <div className="animate-in slide-in-from-top-2 duration-300">
                    <label className="block text-xs font-bold text-gray-500 mb-1">방 비밀번호</label>
                    <input type="password" placeholder="숫자 4자리 이상 입력" value={newRoom.password} onChange={(e) => setNewRoom({...newRoom, password: e.target.value})} className="w-full bg-white border-2 border-black rounded-xl px-4 py-3 text-sm focus:outline-none font-bold" />
                  </div>
                )}
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div><label className="block text-xs font-bold text-gray-500 mb-1">카테고리</label>
                  <select value={newRoom.category} onChange={(e) => setNewRoom({...newRoom, category: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none font-bold appearance-none"><option value="독서모임">독서모임</option><option value="도서교환">도서교환</option></select>
                </div>
                <div><label className="block text-xs font-bold text-gray-500 mb-1">모임 형태</label>
                  <select value={newRoom.type} onChange={(e) => setNewRoom({...newRoom, type: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none font-bold appearance-none"><option value="온라인">온라인</option><option value="오프라인">오프라인</option></select>
                </div>
                <div><label className="block text-xs font-bold text-gray-500 mb-1">최대 인원</label>
              ...
              <div><label className="block text-xs font-bold text-gray-500 mb-1">모임 소개</label><textarea placeholder="모임을 간단히 소개해주세요." rows={3} value={newRoom.desc} onChange={(e) => setNewRoom({...newRoom, desc: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none resize-none"></textarea></div>
              <div><label className="block text-xs font-bold text-gray-500 mb-1">태그 (쉼표로 구분)</label><input type="text" placeholder="예: 인문학, 소설" value={newRoom.tags} onChange={(e) => setNewRoom({...newRoom, tags: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none" /></div>
              <button type="submit" className="w-full mt-4 bg-black text-white font-black py-4 rounded-xl hover:bg-gray-800 transition shadow-lg tracking-widest">개설 완료하기</button>
            </form>
          </div>
        </div>
      )}
