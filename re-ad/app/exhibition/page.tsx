...
export default function ExhibitionPage() {
  ...
  useEffect(() => {
    const fetchExhibitions = async () => {
      setIsLoading(true);
      try {
        // 📍 API 주소도 직관적으로 /exhibition 으로 통일!
    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-24 font-sans">
      ...
      <header className="bg-white/80 backdrop-blur-md px-8 py-4 flex items-center justify-between sticky top-0 z-50 border-b border-gray-100 shadow-sm">
        ...
        <nav className="flex space-x-6 items-center">
          <Link href="/ranking" className="text-sm font-bold text-gray-400 hover:text-black transition">랭킹</Link>
          <Link href="/rooms" className="text-sm font-bold text-gray-400 hover:text-black transition">라운지</Link>
      ...
      <main className="mx-auto max-w-6xl px-6 mt-16">
        <section className="text-center mb-16">
          ...
          <p className="mt-4 text-gray-400 font-bold">우리들의 문장이 전시되는 공간</p>
        </section>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {exhibitions.length === 0 ? (
              <div className="col-span-full text-center py-24 border-2 border-dashed border-gray-200 rounded-[2.5rem]">
                <p className="text-gray-400 font-bold">현재 전시 중인 필사가 없습니다.</p>
                <p className="text-xs text-gray-300 mt-2">첫 번째 필사를 등록해 보세요!</p>
              </div>
            ) : (
              exhibitions.map((item) => (
                <div 
                  key={item.id} 
                  className="bg-white p-8 rounded-[2.5rem] border border-gray-50 flex flex-col justify-between hover:shadow-2xl transition-all group min-h-[300px] relative overflow-hidden"
                >
                  {/* 따옴표 장식 */}
                  <div className="absolute top-6 right-8 text-gray-100 group-hover:text-gray-200 transition-colors">
                  ...
                  <div className="relative z-10">
                    <h4 className="text-[10px] font-black text-gray-400 mb-4 tracking-widest uppercase bg-gray-50 inline-block px-2 py-1 rounded">
                      {item.bookTitle}
                    </h4>
                    <p className="text-xl font-bold leading-relaxed text-gray-800 break-keep">
                      "{item.content}"
                    </p>
                  </div>

                  <div className="mt-8 pt-6 border-t border-gray-50 flex items-center justify-between">
                    <span className="text-xs font-black text-gray-400">By {item.author}</span>
